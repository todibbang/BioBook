...
public class Seat {
    ...
    public static ArrayList<Seat> getAllFreeSeatsForShowing(int showingId){ 
        ...
        //ArrayList<ArrayList<String>> seatString = MySQL.getInstance().executeQuery("SELECT * FROM seats LEFT JOIN showingReservations ON seats.seatId = showingReservations.seatId UNION ALL SELECT * FROM seats RIGHT JOIN showingReservations ON seats.seatId = showingReservations.seatId WHERE showingReservations.showingId = "+ showingId);
        //ArrayList<ArrayList<String>> seatString = MySQL.getInstance().executeQuery("SELECT * FROM seats LEFT JOIN showings ON seats.roomId = showings.roomId WHERE showings.showingId = " + showingId + " AND seats.seatId NOT IN (SELECT seatId FROM showingReservations)"  + " LIMIT 40");
